...
def gaussian_kernel2d(
    ...
    Returns:
    -----------
        torch.Tensor: tensor of shape (1, 1, win_size, win_size)
    """
    ...
    kernel_2d = kernel_2d.expand(
        n_channels, 1, window_size, window_size
    )
...
def sobel_hv(window_size: int = 5, device: torch.device = None):
    ...
    ----------
        ...
        direction (str): 
            direction of the derivative. One of ("x", "y")

    Returns:
    ----------
        torch.Tensor: the computed 1st order derivatives of the input 
        tensor. Result shape (B, 2, H, W)
    """
