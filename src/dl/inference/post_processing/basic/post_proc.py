...
def shape_index_watershed(
    ...
    """
    After thresholding, this function can be used to post process each 
    nuclei instance. Uses shape index to find local curvature from the 
    probability map and uses that information to separate overlapping 
    nuclei.
    ...
    for nuc_id in nuc_list:
        ...
        for sub_nuc_id in cell_ids:
            ...
            sub_mask = filters.rank.median(
                util.img_as_ubyte(sub_mask), morph.disk(3)
            )
            ...
            # if cells end up overlapping after dilations then remove 
            # the overlaps so no new ids are created when summing 
            # overlapping ids to the result mask
...
def sobel_watershed(
    ...
    """
    After thresholding, this function can be used to compute distance 
    maps for each nuclei instance and watershed segment the elevation 
    map of the prob_map (sobel). Before computing distance maps a binary
    opening is performed to the instance map. Markers for the distance 
    maps are computed using niblack thresholding on the distance map.
    ...
    ----------
        ...
        win_size (int, default=13): 
            window size used in niblack thresholding the distance maps 
            to find markers for watershed
...
def inv_dist_watershed(
    ...
    """
    After thresholding, this function can be used to compute distance 
    maps for each nuclei instance and watershed segment the inverse
    distmaps. Before computing distance maps a binary opening is 
    performed to the instance map. Markers for the distance maps are 
    computed using niblack thresholding on the distance map.
