# Ported from https://gist.github.com/ananthsub/b9e23f480cd993945d2f3fc35f0520f9
...
class LayerSummary(object):
    ...
    def _register_hook(self) -> RemovableHandle:
        ...
        def hook(module, inp, out):
            ...
            self._flops = int(_calculate_flops(module, inp, out))
    ...
    @property
    def num_flops(self) -> Union[float, str]:
        return self._flops or 0
...
class ModelSummary(object):
    ...
    @property
    def flops(self) -> List:
        return [layer.num_flops for layer in self._layer_summary.values()]
    ...
    def __str__(self):
        ...
        arrays = [
            ...
            ["Flops", list(map(get_human_readable_count, self.flops))],
        ]
        if self._model.example_input_array is not None:
...
def _calculate_flops(module: nn.Module, inp, out) -> Union[float, int]:
    layer_type = module.__class__.__name__
    if layer_type == "Conv1d":
        # x shape is N x C x W
        out_w = int(
            (inp.size()[2] + 2 * module.padding[0] - module.kernel_size[0])
            / module.stride[0]
            + 1
        )
        return (
        ...
        out_w = int(
            (inp.size()[3] + 2 * module.padding[1] - module.kernel_size[1])
            / module.stride[1]
            + 1
        )
        return (
    ...
    elif layer_type == "LearnedGroupConv":
        conv = module.conv
        out_h = int(
            (inp.size()[2] + 2 * conv.padding[0] - conv.kernel_size[0]) / conv.stride[0]
            + 1
        )
        out_w = int(
            (inp.size()[3] + 2 * conv.padding[1] - conv.kernel_size[1]) / conv.stride[1]
            + 1
        )
        count1 = _calculate_flops(module.relu, inp, None) + _layer_flops(
            module.norm, inp, None
        )
        count2 = (
            conv.in_channels
            * conv.out_channels
