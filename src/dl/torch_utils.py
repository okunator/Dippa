...
def ndarray_to_tensor(array: np.ndarray) -> torch.Tensor:
    """
    Convert img or mask (H, W) or (H, W, C) to tensor (1, C, H, W)
    """
    assert isinstance(array, np.ndarray), f"Input type: {type(array)} is not np.ndarray"
    assert 1 < len(array.shape) <= 3, f"ndarray.shape {array.shape}, rank needs to be 2 or 3" 
...
def tensor_to_ndarray(tensor: torch.Tensor, channel: int = None) -> np.ndarray:
    """
    Convert img or network output tensor (B, C, H, W) to ndarray (H, W, C)
    """
    ...
    if channel is not None:
        res_tensor = res_tensor.numpy().transpose(0, 2, 3, 1).squeeze()[..., channel]
    else:
        res_tensor = res_tensor.numpy().transpose(0, 2, 3, 1).squeeze()
...
def one_hot(type_map: torch.Tensor, n_classes: int) -> torch.Tensor:
    ...
    """
    return torch.eye(n_classes)[:, type_map.long()]
