...
from src.dl.models.decoders.decoder_stage import DecoderStage
...
def test_forward(
    ...
    """
    Test the forward method of one decoder stage.
    """

    in_channels = dec_channels[stage_ix]
    in_dim = dec_out_dims[stage_ix]
    sample = _get_in_sample(in_channels, in_dim)
    ...
    with torch.no_grad():
        out, extras = dec_stage(x=sample, ix=stage_ix, skips=skips, extra_skips=extra_skips)

    assert out.shape[1] == dec_stage.out_channels
    assert out.shape[2:] == up(sample).shape[2:]
...
def test_forward_backward(
    ...
    """
    Test the forward and backward method of decoder stage.
    """
    ...
    out, _ = dec_stage(x=sample, ix=stage_ix, skips=skips, extra_skips=extra_skips)
    out.mean().backward()
